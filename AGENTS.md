...
- **Skills** — `agent/skill.dart`. Two mutually-exclusive modes per agent: pure-Dart `Skill` objects (`skills:`) **or** filesystem `SKILL.md` discovery (`skillDirectoryPaths:`). Filesystem skills can run JS via an injected `JavaScriptRuntime`.
